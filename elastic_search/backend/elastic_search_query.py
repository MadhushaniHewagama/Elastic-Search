...
class ElasticSearchQuery :
    ...
    def process(self,query):
        ...
        if("view" in boosts_params and len(boosts_params)>1):
            sortByview = True
            res = es.search(index='sinhala_songs',body = self.matching(query_mod,boosts,sortByview))
        elif("view" in boosts_params and len(boosts_params)==1):
            sortByview = True
            if(len(tokens) == 1):
                res = es.search(index='sinhala_songs',body = self.all_matching_best(sortByview))
            else:
                res = es.search(index='sinhala_songs',body = self.matching(query_mod,boosts,sortByview))
        else:
            sortByview = False
